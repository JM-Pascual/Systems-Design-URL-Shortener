//! Tier 3 — persistence. Same two endpoints as Tier 1, backed by Postgres
//! instead of an in-process `HashMap`. Run it with:
//!
//! ```text
//! docker compose -f tier-3-persistence/docker-compose.yml up -d
//! cargo run -p tier-3-persistence
//! ```
...
///
/// No `Mutex` here, unlike Tier 1 — `Store` wraps a `PgPool`, which is
/// already safe to share across concurrent requests. See `store.rs` for why.
struct AppState {
    store: Store,
    config: Config,
}
...
/// table. Meant for watching the table fill up during a demo, not as an API
/// a real client would parse.
...
async fn main() {
    ...
    println!("tier-3-persistence listening on 0.0.0.0:3000");
