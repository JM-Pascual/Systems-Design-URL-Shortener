...
pub struct Config {
    ...
    /// Redis connection string, e.g. `redis://localhost:6383`.
...
/// `docker compose up -d`.
const DEFAULT_DATABASE_URL: &str = "postgres://shortener:shortener@localhost:5437/shortener";

/// Matches `docker-compose.yml`'s Redis port.
const DEFAULT_REDIS_URL: &str = "redis://localhost:6383";
